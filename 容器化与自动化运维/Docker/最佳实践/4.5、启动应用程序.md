...
## 显示使用文档
**不启动任何应用程序，只是打印出使用说明，比如s2i的构建器镜像，只负责编译**
基于[[#生成构建器镜像]]，docker run xxx构建器镜像

## 容器内部，使用systemd启动应用
**需要两步**
1. `RUN systemctl enable rsyslog` 通知systemd启用该服务
2. `RUN /usr/sbin/init` 交给init进程处理剩余工作

1. 以centos为基础镜像，安装一些缺少的工具
Dockerfile_Systemd_Base
```Dockerfile
FROM centos
...
* 缺少selinux-policy-targeted安装
[ERROR: Unable to open policy //etc/selinux/targeted/policy/policy.31](https://dbalifeeasy.com/2020/09/14/error-unable-to-open-policy-etc-selinux-targeted-policy-policy-31/)

`docker build -t centos:systemd -f Dockerfile_Systemd_Base .`

2. 编写启动Systemd服务的Dockerfile
Dockerfile_Systemd_Demo
```Dockerfile
FROM centos:systemd
...
`docker build -t systemd-demo -f Dockerfile_Systemd_Demo .`

3. 启动容器
`docker run -privileged --rm systemd-demo`
**这个教程说：从Docker1.0开始，无须在docker run带上--privileged参数
是错误的**[[Docker重点]]
[Docker官方release日志还有这个参数讲解，Engine 1.13 and earlier](https://docs.docker.com/engine/release-notes/prior-releases/)

**启动直接导致docker命令无法使用，重启docker也不行，只能重启主机**
```shell
...
-- 
-- The result is failed.
8月 08 13:06:44 basic systemd[1]: Unit docker.service entered failed state.
8月 08 13:06:44 basic systemd[1]: docker.service failed.
8月 08 13:06:46 basic systemd[1]: docker.service holdoff time over, scheduling restart.
8月 08 13:06:46 basic systemd[1]: Stopped Docker Application Container Engine.
-- Subject: Unit docker.service has finished shutting down
-- Defined-By: systemd
-- Support: http://lists.freedesktop.org/mailman/listinfo/systemd-devel
-- 
-- Unit docker.service has finished shutting down.
8月 08 13:06:46 basic systemd[1]: start request repeated too quickly for docker.service
8月 08 13:06:46 basic systemd[1]: Failed to start Docker Application Container Engine.
...
-- 
-- The result is failed.
8月 08 13:06:46 basic systemd[1]: Unit docker.service entered failed state.
8月 08 13:06:46 basic systemd[1]: docker.service failed.
```

## 使用主机Systemd管理容器的启动顺序[[Docker重点]]1
常规的容器顺序启动：
1. docker提供的docker run --restart参数
2. 编排平台，如kubernetes
3. Systemd
service模板
```
