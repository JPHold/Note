...
**下面自己探索出原理(核心是docker cp与tar标准输入输出流的联动)**

1. 创建容器
`docker create --name s2i_multiple_build_image s2i_multiple_build_image:latest "/bin/sh" "-c" '/usr/libexec/s2i/assemble'`
CMD为/usr/libexec/s2i/assemble

2. 创建/tmp/src
因为s2i构建器镜像，操作的目录是/tmp/src，所以要确保存在该目录
通过inspect镜像信息，GraphDriver.Data.UpperDir得知（是当前容器层的存放目录）：
```
mkdir -p `docker inspect -f '{{.GraphDriver.Data.UpperDir}}' b19b13e23f91\`/tmp/src`
```

3. 拷贝源代码到该容器
* 切换到源代码目录，执行
`tar -cf - . | docker cp - d15c82b16d99:/tmp/src`
