...
| [计算属性与侦听器](https://v2.cn.vuejs.org/v2/guide/computed.html)| **计算属性是缓存的，基于它们的响应式依赖进行缓存的** |
...
| [render渲染函数+JSX语法](https://v2.cn.vuejs.org/v2/guide/render-function.html)                                                                                                                        | 大部分使用模板，编译成虚拟渲染函数，即可满足需求，如果要自己控制，就跨过模板自己写渲染函数。采用组件+render函数+createElement函数+[v-slot插槽](https://v2.cn.vuejs.org/v2/api/#%E5%AE%9E%E4%BE%8B-property)，this.$slots.default代表没有插槽包裹（没有用v-slot声明的普通元素）或`v-slot:default`的内容 | this.$slots.default的显示，是优先级的，多个v-slot:default则显示最后一个；有v-slot:default声明的元素，则没声明的元素不会显示              |    |
