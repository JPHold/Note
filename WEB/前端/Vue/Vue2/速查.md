...
## 局部组件
### 特性
1. template模板，如果有多个元素，记得使用div包裹，不然只会显示第一个元素 
2. template模板内容，支持插槽：`<slot>`;  在使用该组件时，除了固定模板，还可以加上自定义的html内容，适应变化
3. props，外部传递参数到模板内部
4. $emit，组件通过 `this.\$emit('xxxFunction', xxxParam...)` 与外部进行通信
