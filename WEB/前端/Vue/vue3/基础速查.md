...
| [侦听器] | 1. 根据某个状态变更，做相应操作，如请求数据、更新DOM<br>2. watch(xxx要监听的值，(新值，旧值)函数)，监听的值类型可以是ref实例、reactive响应式对象、get函数、数组；不能监听响应式对象的属性值<br>3. 支持深层结构的响应式对象的变动更新，但如果是监听返回响应对象的get函数，则不支持（只有不同对象实例才会触发更新，可通过`{ deep: true }`强制）<br>4. **注意，别监听很多层次、很大的响应式对象，会遍历所有嵌套的属性，性能很差**<br>5. 通过`{immediate: true}`，创建侦听器，就立马触发更新回调[[#侦听器]] |
