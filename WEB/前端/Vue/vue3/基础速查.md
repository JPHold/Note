...
| [ref函数](https://cn.vuejs.org/api/reactivity-core.html#ref) | 定义响应式变量。<br>1. ref()解决了reactive()的问题；<br>2. 支持所有类型（基础类型、对象类型）的数据作为响应式对象；<br>3. 赋值、解构到本地变量或传递到函数中而不失去响应。<br> 4. 要利用好解包（解构）[[#ref()]]                        |                                                                                                                                                                                                          |      |
