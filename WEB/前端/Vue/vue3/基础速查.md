...
| 表单输入绑定 |<br>1. v-model等于v-bind:value和@input的组合简写<br>2. `<input>` 和 `<textarea>` 元素是绑定value属性和监听input事件；`input type="checkbox">` 和 `<input type="radio">`是绑定checked属性和监听change事件；`<select>`是绑定value属性和监听change事件<br>3.   |
