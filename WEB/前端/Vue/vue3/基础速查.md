...
| [计算属性：computed](https://cn.vuejs.org/guide/essentials/computed.html)                            | <br>1. 根据被计算的属性值，生成新值，而且是响应式的；<br>2. 缓存特性：被计算的属性值未变则不会重新计算，而方法是每次渲染都重新计算<br>3. 不要在方法中执行异步操作和DOM操作<br>[[#计算属性]]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |     |     |
...
| [列表渲染](https://cn.vuejs.org/guide/essentials/list.html)                                                                                                     | <br>1. 遍历数组有三种写法，vue写法：`v-for="(item, index) in items"`、js原生解构写法：`v-for="{ message } in items"、v-for="({ message }, index) in items"`、js原生of写法：`v-for="item of items"`<br>2. 遍历对象：获取值`v-for="value in myObject"`、获取值、键`v-for="(value, key) in myObject"`、获取值、键、位置`v-for="(value, key, index) in myObject"`<br>3. 语法糖-遍历范围值,从1遍历到10`v-for="n in 10"`4. 搭配`<template>`批量生成多个元素5、默认                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |     |     |
