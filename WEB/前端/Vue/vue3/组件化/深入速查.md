...
| [异步组件](https://cn.vuejs.org/guide/components/async.html) |<br>1. 在复杂项目中，一个页面加载大量组件，会加载缓慢，导致大面积空白<br>2. 可将某些组件在需要时才加载，加快界面渲染展示<br>3. 使用`defineAsyncComponent`，原理是定义多一层包装层（xxxWrapper），包裹源组件，跟普通组件一样使用，定义的prop和插槽都会传递[[#defineAsyncComponent]]  |
# 注册
## 全局注册
main.js
```js
...
点击按钮，才渲染组件；**v-if是每次都渲染，但在这组件只加载一次**
AsyncComp.vue
```js
<script setup>
