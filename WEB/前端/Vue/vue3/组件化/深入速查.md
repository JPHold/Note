...
| 事件 | <br>1. 组件定义的事件无冒泡进制。平级组件或是跨越多层嵌套的组件间通信，应使用一个外部的事件总线，或是使用一个[全局状态管理方案](https://cn.vuejs.org/guide/scaling-up/state-management.html)。<br>2. 支持事件参数，`@xxx-xxx="(...)=>...."` `$emit('xxx-xxx', ...)`<br>3. 可以声明事件也可以不声明，声明的话有这些好处：可以在任何处理中触发事件和参数校验 |
# 注册
## 全局注册
main.js
```js
