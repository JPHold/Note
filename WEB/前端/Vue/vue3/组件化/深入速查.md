...
| [插槽slot]() | <br>1. 灵感来自于[原生 Web Component `<slot>` 元素](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) <br>2. 插槽内容无法访问子组件的数据，只能访问定义他的父组件的数据；这遵守vue模板中的表达式只能访问其定义时所处的作用域（也就是JavaScript词法的作用域）<br>3. 支持插槽内容默认值，在`<slot></slot>`之间写即可<br>4. 支持指定渲染到指定插槽，子组件在`<slot>`声明`name`属性：如xxx，父组件用`<template>`分类包裹插槽内容，并加上`v-slot='xxx'`命令（简写是`#xxx`，xxx跟子组件的`<slot>`的name属性对应）；**`v-slot='default'`为默认插槽内容，插入到没有声明`name`的`<slot>`中**|
