...
| 内置组件 | <br>1. [Transition](https://cn.vuejs.org/guide/built-ins/transition.html) 用于`v-if、v-show、<component :is="xxxComponent">` 触发的DOM元素进入和移除触发的动画效果，具体到进入、中途过渡、退出<br>2. [TransitionGroup](https://cn.vuejs.org/guide/built-ins/transition-group.html) 跟Transition类似，只不过是用于`v-for`触发每一项元素进入和移除触发的动画效果<br>3. [KeepAlive](https://cn.vuejs.org/guide/built-ins/keep-alive.html)，`<component :is="xxxComponent">`切换组件时，如果组件有状态的（如填写的DOM元素等），那么切换后所有东西都销毁了。这时用`<KeepAlive>`包裹，即可保存状态;<br> 通过`include` 和 `exclude`属性定义哪些组件要缓存，哪些不要，支持逗号分隔的字符串、正则、数组，这些值是组件的名称，所以需要定义name属性，但SFC单文件组件不用，文件名即为name<br>4. 通过max属性指定最大缓存的组件实例数：**当达到时，最久没被访问的组件实例会被销毁**<br>5.  [`onActivated()`](https://cn.vuejs.org/api/composition-api-lifecycle.html#onactivated) 和 [`onDeactivated()`](https://cn.vuejs.org/api/composition-api-lifecycle.html#ondeactivated)是组件状态变更的回调，分别在首次挂载到DOM或加载缓存并显示、首次从DOM卸载或从DOM移除并重新进入缓存[[#缓存实例的生命周期]]|
