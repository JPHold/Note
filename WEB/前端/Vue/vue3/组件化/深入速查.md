...
| [依赖注入](https://cn.vuejs.org/guide/components/provide-inject.html) |<br>1. 避免组件树很深，要一层层传递props，很不方便，因此可以使用`provide`和`inject`<br>2. inject支持默认值，即使存在属性值，也会执行第二个参数，导致浪费性能，需要用到工厂方法：`()=>`[[#默认值]]<br>3. 注入方可以修改提供方的属性（声明props是不可修改），不要直接修改，最好是定义修改函数并注入使用，或者不允许修改：[[#通过readonly声明不可修改]]<br>4. 复杂项目中，会引用许多依赖（如使用第三方库），那么就会出现名称一样的实例，所以使用Symbol类型来解决冲突  |
# 注册
## 全局注册
main.js
```js
