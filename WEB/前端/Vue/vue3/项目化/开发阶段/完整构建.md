...
| [状态管理](https://cn.vuejs.org/guide/scaling-up/state-management.html) | <br>1. 一般是用js文件封装ref、reactive响应式对象来定义共享状态，然后需要共享的组件，引入他即可<br>2. 但只适用于SPA，不适用于服务器渲染，且存在安全漏洞。适用[pina](https://pinia.vuejs.org/zh/introduction.html) |
| 服务端渲染（SSR） | <br>1. vue+node.js+express服务发布+createSSRApp函数+renderToString函数，访问对应url，显示对应静态内容 |
